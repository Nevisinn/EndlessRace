...
public class CarController : MonoBehaviour
{
    ...
    private int maxSpeed; //The maximum speed that the car can reach in km/h.

    [Range(10, 120)]
    public int maxReverseSpeed = 45; //The maximum speed that the car can reach while going on reverse in km/h.

    // [Range(1, 10)]
    public int accelerationMultiplier; // How fast the car can accelerate. 1 is a slow acceleration and 10 is the fastest.
    ...
    */
    ...
    void Update()
    {
        ...
        */
        if (useTouchControls && touchControlsSetup)
        {
            ...
            if (
                (!reversePTI.buttonPressed && !throttlePTI.buttonPressed)
    ...
    public void Handbrake()
    {
        ...
        if (driftingAxis < 1f)
        {
            FLwheelFriction.extremumSlip =
                FLWextremumSlip * handbrakeDriftMultiplier * driftingAxis;
            frontLeftCollider.sidewaysFriction = FLwheelFriction;

            FRwheelFriction.extremumSlip =
                FRWextremumSlip * handbrakeDriftMultiplier * driftingAxis;
            frontRightCollider.sidewaysFriction = FRwheelFriction;

            RLwheelFriction.extremumSlip =
                RLWextremumSlip * handbrakeDriftMultiplier * driftingAxis;
            rearLeftCollider.sidewaysFriction = RLwheelFriction;

            RRwheelFriction.extremumSlip =
                RRWextremumSlip * handbrakeDriftMultiplier * driftingAxis;
    ...
    public void DriftCarPS()
    {
        if (useEffects)
        {
            ...
            {
                if (
                    (isTractionLocked || Mathf.Abs(localVelocityX) > 5f)
                    && Mathf.Abs(carSpeed) > 12f
                )
                {
